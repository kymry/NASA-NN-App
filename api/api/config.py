...
class Config:
    ...
    DATABASE = os.environ.get('DATABASE') or os.path.join(basedir, 'databases/marsweather.db')
    # 'sqlite:///' tells SQLAlchemy which database engine to use
    SQLALCHEMY_DATABASE_URI = os.environ.get('DATABASE_URL') or \
                              'sqlite:///' + os.path.join(basedir, 'databases/marsweather.db')
    SQLALCHEMY_TRACK_MODIFICATIONS = False  # if True, signals a change every time an update is made to the db
